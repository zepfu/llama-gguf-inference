...
**Why this works:** Platform health checks on a separate port don't count as "activity" for idle timeout purposes,
allowing proper scale-to-zero behavior.
...
**Note:** For serverless deployments, configure your platform to use `PORT_HEALTH` (8001) for health checks instead of
`/ping`. This enables proper scale-to-zero behavior by separating platform monitoring from actual API traffic.
