...
4. **Check VRAM usage** (from boot log):
   ```
...
2. **Try DEBUG_SHELL=true** — Inspect container state
3. **Try NGL=0** — Isolate GPU issues
4. **Simplify** — Minimal config, small model
...
4. GPU type
5. Platform (RunPod/Vast.ai/local/etc)
6. Error messages
